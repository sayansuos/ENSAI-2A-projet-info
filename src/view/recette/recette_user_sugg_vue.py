...
class RecetteUserSuggVue(VueAbstraite):
    """
    Vue qui affiche toutes les recettes suggérées pour l'utilisateur connecté.
    """
    ...
    def choisir_menu(self):
        ...
        if recettes == []:
            print("\n\nWe're sorry, there is no suggested recipe for you...")
            print("Maybe you should try to add more favourite ingredients!\n\n")
            inquirer.select(message="", choices=["Ok"]).execute()
            return MenuUserVue(message=self.message, utilisateur=self.utilisateur)

        # Sinon on les montre et on propose les actions associées :
        else:
            choix = "-> Page suivante"
            i = 0
